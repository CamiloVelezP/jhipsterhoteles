...
export const Hotel = () => {
  ...
  const hotelList = useAppSelector(state => state.apigw.hotel.entities);
  const loading = useAppSelector(state => state.apigw.hotel.loading);
  const totalItems = useAppSelector(state => state.apigw.hotel.totalItems);
    ...
    <div>
      ...
      <div className="table-responsive">
        {hotelList && hotelList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('cadena')}>
                  Cadena <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('ciudad')}>
                  Ciudad <FontAwesomeIcon icon="sort" />
                </th>
                ...
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/hotel/${hotel.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`/hotel/${hotel.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">Ningún Hotels encontrado</div>
        )}
      </div>
      {totalItems ? (
