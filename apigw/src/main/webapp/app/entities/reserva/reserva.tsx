...
export const Reserva = () => {
  ...
  const reservaList = useAppSelector(state => state.apigw.reserva.entities);
  const loading = useAppSelector(state => state.apigw.reserva.loading);
  const totalItems = useAppSelector(state => state.apigw.reserva.totalItems);
            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('fechaInicio')}>
                  Fecha Inicio <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('fechaFin')}>
                  Fecha Fin <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  Id Habitacion <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  Nro Doc <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {reservaList.map((reserva, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{reserva.fechaInicio ? <TextFormat type="date" value={reserva.fechaInicio} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{reserva.fechaFin ? <TextFormat type="date" value={reserva.fechaFin} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>
                    {reserva.idHabitacion ? <Link to={`/habitacion/${reserva.idHabitacion.id}`}>{reserva.idHabitacion.id}</Link> : ''}
                  </td>
                  <td>{reserva.nroDoc ? <Link to={`/usuario/${reserva.nroDoc.id}`}>{reserva.nroDoc.id}</Link> : ''}</td>
