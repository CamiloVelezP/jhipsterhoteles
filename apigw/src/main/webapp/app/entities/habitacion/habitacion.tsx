...
export const Habitacion = () => {
  ...
  const habitacionList = useAppSelector(state => state.apigw.habitacion.entities);
  const loading = useAppSelector(state => state.apigw.habitacion.loading);
  const totalItems = useAppSelector(state => state.apigw.habitacion.totalItems);
  ...
  return (
    <div>
            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('ubicacion')}>
                  Ubicacion <FontAwesomeIcon icon="sort" />
                </th>
                ...
                <th>
                  Id Hotel <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {habitacionList.map((habitacion, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{habitacion.disponible ? 'true' : 'false'}</td>
                  <td>{habitacion.idHotel ? <Link to={`/hotel/${habitacion.idHotel.id}`}>{habitacion.idHotel.id}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/habitacion/${habitacion.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`/habitacion/${habitacion.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
      ...
      {totalItems ? (
        <div className={habitacionList && habitacionList.length > 0 ? '' : 'd-none'}>
